...
{
    if ( [self colorFromName:hexString] )
        return [self colorFromName:hexString];
...
{
    CGFloat hue = 0, saturation = 0, lightness = 0, alpha = 0;
...
+ (UIColor *)color:(UIColor *)color_ withAlpha:(float)alpha_ {
    ...
    [UIColor getRGBComponents:components forColor:color_];
    red = components[0];
    green = components[1];
    blue = components[2];
