...
static char kWRSystemNavDefaultBarBarButtonItemKey;    // 默认返回按钮
...
// 默认返回按钮
+ (UIBarButtonItem *)wr_defaultBarButtonItem {
    return (UIBarButtonItem *)objc_getAssociatedObject(self, &kWRSystemNavDefaultBarBarButtonItemKey);
}

+ (void)wr_setDefaultBarButtonItem:(UIBarButtonItem *)barButtonItem {
    objc_setAssociatedObject(self, &kWRSystemNavDefaultBarBarButtonItemKey, barButtonItem, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)wr_setBackgroundAlpha:(CGFloat)alpha {
    ...
    {   // sometimes we can't change _UIBarBackground alpha
        ...
        if ([barBackgroundView isKindOfClass:NSClassFromString(@"_UIBarBackground")]) {
            barBackgroundView = [barBackgroundView subviews].firstObject;
        }
...
static char kWRNavigationBarHiddenKey;
...
static char kWRSystemNavBarLeftBarButtonItemKey;
...
- (UIBarButtonItem *)wr_leftBarButtonItem {
    return (UIBarButtonItem *)objc_getAssociatedObject(self, &kWRSystemNavBarLeftBarButtonItemKey);
}

- (void)wr_setLeftBarButtonItem:(UIBarButtonItem *)barButtonItem {
    objc_setAssociatedObject(self, &kWRSystemNavBarLeftBarButtonItemKey, barButtonItem, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (BOOL)wr_navigationBarHidden {
    id hidden = objc_getAssociatedObject(self, &kWRNavigationBarHiddenKey);
        return (hidden != nil) ? [hidden boolValue] : NO;
}

- (void)wr_setNavigationBarHidden:(BOOL)hidden {
    objc_setAssociatedObject(self, &kWRNavigationBarHiddenKey, @(hidden), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    [self setNeedsStatusBarAppearanceUpdate];
}
...
- (void)wr_viewWillAppear:(BOOL)animated {
    if ([self canUpdateNavigationBar]) {
        ...
        [self.navigationController setNavigationBarHidden:[self wr_navigationBarHidden] animated:YES];
    }
...
- (void)wr_viewDidLoad {
    ...
    if (self.navigationController.viewControllers.count > 1) {
        if ([self wr_leftBarButtonItem]) {
            [self.navigationItem setLeftBarButtonItem:[self wr_leftBarButtonItem]];
        } else if ([WRNavigationBar wr_defaultBarButtonItem]) {
            [self.navigationItem setLeftBarButtonItem:[WRNavigationBar wr_defaultBarButtonItem]];
        }
    }
}
...
- (void)wr_viewDidAppear:(BOOL)animated {
    ...
    if ([self canUpdateNavigationBar] == YES)
    {
        ...
        if ([self wr_statusBarStyle]) {
            [UIApplication sharedApplication].statusBarStyle = [self wr_statusBarStyle];
        } else {
            [UIApplication sharedApplication].statusBarStyle = [WRNavigationBar defaultStatusBarStyle];
        }
    }
