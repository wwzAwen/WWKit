...
- (void)requestImage:(void (^)(ZZFDAlbumModel *model, UIImage *image))imageAction
{
    if (self.image) {
        ...
        [[PHImageManager defaultManager] requestImageForAsset:self.asset targetSize:CGSizeMake(kScreenWidth / 2, kScreenWidth / 2) contentMode:PHImageContentModeAspectFill options:nil resultHandler:^(UIImage *result, NSDictionary *info) {
            self.image = result;
            if (imageAction) {
                imageAction(self, self.image);
            }
