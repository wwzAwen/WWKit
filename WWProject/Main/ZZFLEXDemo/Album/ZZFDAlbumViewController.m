    ...
    [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (status == PHAuthorizationStatusAuthorized) {
                ...
                [UIAlertTool showAlertWithTitle:@"未获取到相册权限" message:@"请在设置/ZZFLEXDemo/照片-允许访问照片" cancelButtonTitle:@"取消" otherButtonTitles:@[@"确定"] actionHandler:^(NSInteger buttonIndex) {
                    if (buttonIndex == 1) {
...
- (void)loadListWithData:(PHFetchResult<PHAsset *> *)assetData
{
    ...
    CGFloat space = 2;
    CGFloat itemWidth = (kScreenWidth - space * (lineCount + 1)) / lineCount;
