...
- (void)setViewDataModel:(ZZFLEXEditModel *)dataModel
{
    ...
    self.angel.addCells([ZZFDCateItemCell class]).toSection(1).withDataModelArray(dataModel.titleModel).selectedAction(^ (ZZFDPlatformItemModel *model) {
        ...
        if (!model.selected && [self selectedCount] == FDCATE_MAX_COUNT) {
            [UIAlertTool showAlertWithTitle:@"已达选择上限" message:nil cancelButtonTitle:@"确定"];
        }
        else {
...
{
    CGFloat width = (kScreenWidth - 15 * 5) / 4;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        .backgroundColor(UIColorFromRGB(239.0, 239.0, 244.0)).titleColor([UIColor grayColor])
        .backgroundColorSelected([UIColor colorWithHex:@"#ff5647" alpha:1.0]).titleColorSelected([UIColor whiteColor])
        .masonry(^(UIView *senderView, MASConstraintMaker *make) {
