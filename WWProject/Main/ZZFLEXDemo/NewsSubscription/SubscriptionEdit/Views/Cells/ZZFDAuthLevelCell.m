...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        .borderWidth(0.5).borderColor([UIColor colorWithWhite:0.5 alpha:0.3].CGColor).cornerRadius(2.0f)
        ...
        .backgroundColor([UIColor colorWithWhite:0.5 alpha:0.3])
        .masonry(^(UIView *senderView, MASConstraintMaker *make) {
            make.right.mas_equalTo(self.maxTF.mas_left).mas_offset(-10);
            make.centerY.mas_equalTo(0);
            make.height.mas_equalTo(0.5 * 2);
