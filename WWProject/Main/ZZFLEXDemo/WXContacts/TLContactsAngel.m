...
- (void)resetListWithContactsData:(NSArray *)contactsData sectionHeaders:(NSArray *)sectionHeaders
{
    ...
    {
        TLContactsItemModel *newModel = createContactsItemModelWithTag(TLContactsVCCellTypeNew, @"friends_new", nil, kLocStr(@"新的朋友"), nil, nil);
        TLContactsItemModel *groupModel = createContactsItemModelWithTag(TLContactsVCCellTypeGroup, @"friends_group", nil, kLocStr(@"群聊"), nil, nil);
        TLContactsItemModel *tagModel = createContactsItemModelWithTag(TLContactsVCCellTypeTag, @"friends_tag", nil, kLocStr(@"标签"), nil, nil);
        TLContactsItemModel *publicModel = createContactsItemModelWithTag(TLContactsVCCellTypePublic, @"friends_public", nil, kLocStr(@"公众号"), nil, nil);
    ...
    if (sectionModel.sectionTag != TLContactsVCSectionTypeFuncation && sectionModel.sectionTag != TLContactsVCSectionTypeEnterprise) {
        ...
        remarkAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleNormal
                                                        ...
                                                        handler:^(UITableViewRowAction *action, NSIndexPath *indexPath) {
                                                            [MBProgressHUD showSuccessMessage:@"备注：暂未实现"];
                                                        }];
        return @[remarkAction];
    }
    return @[];
